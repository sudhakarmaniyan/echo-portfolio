...
const Folder = ({ color = '#5227FF', size = 1, items = [], className = '' }: FolderProps) => {
  // Support up to 7 fanning items for projects
  const maxItems = 7;
  const papers = items.slice(0, maxItems);
  
  // Fill with empty placeholders if we need more papers to make it look full (optional)
  while (papers.length < 3) {
    papers.push(<div style={{ width: '100%', height: '100%', background: '#fff' }} />);
  }
